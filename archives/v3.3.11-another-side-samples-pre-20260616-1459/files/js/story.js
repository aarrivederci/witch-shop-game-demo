...
const Story = (() => {
  ...
  let _activeArchiveView = 'stories';
  ...
  function render() {
    ...
    container.appendChild(_buildArchiveViewTabs());

    if (_activeArchiveView === 'records') {
  ...
  function _buildArchiveViewTabs() {
    const tabs = document.createElement('div');
    tabs.className = 'story-archive-tabs';
    [
      { id: 'stories', label: '故事' },
      { id: 'records', label: '记录' },
    ].forEach(view => {
      const button = document.createElement('button');
      button.className = 'story-archive-tab' + (_activeArchiveView === view.id ? ' active' : '');
      button.textContent = view.label;
      button.addEventListener('click', () => {
        _activeArchiveView = view.id;
        render();
      });
  ...
  function _renderRecords(container) {
    const records = _getStoryRecordCards();
    const header = document.createElement('div');
    header.className = 'story-record-header';
    header.innerHTML = `
      <div class="story-record-title">📚 小店记录页</div>
      <div class="story-record-desc">这里只展示职业与角色维度的阶段记录；它们是半隐藏线索，不是当前统计面板。</div>
    `;
    ...
    if (!records.length) {
      const empty = document.createElement('div');
      empty.className = 'story-empty';
      empty.textContent = '暂时还没有形成可阅读的阶段记录。继续经营后，柜台上的账页会慢慢出现折痕。';
      container.appendChild(empty);
      return;
    }
  ...
  /* ─── PUBLIC ─── */
  return { render, checkUnlocks, getUnlockHints, getUnlockConditions };
})();
