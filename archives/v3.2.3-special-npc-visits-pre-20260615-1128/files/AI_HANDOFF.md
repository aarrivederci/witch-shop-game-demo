...
建议先判断是否还要在 v3.2.2 做 `sampleOnUnlock` 样品库存；如果暂不做库存，可直接进入 v3.2.3：订单顾客区分一般 NPC 与特殊 NPC；特殊 NPC 使用已有 20 个具名角色并根据羁绊等级显示不同来访台词；同一时间最多 1 个特殊 NPC 订单，失败回退一般 NPC。羁绊阶段回馈第一版先做 `noteOnly` 记录和提示。暂不继续新增故事条件字段、暂不重构故事预告、暂不扩写流失订单/IF 故事网络、暂不做完整特殊委托链。
