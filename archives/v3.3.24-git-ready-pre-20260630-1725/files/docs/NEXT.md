...
- **scope**：在仓库根 `git init`，把 `archives/` 纳入但用 git-lfs / shallow strategy 控制体积，给文件级归档加一层差异追踪。
- **blockers**：用户授权；归档目录已有约 250 MB 量级估算（48 个版本 + 完整快照），需先确认体积可控。
- **archive_to_make**：`v3.x-git-init-pre-<ts>` 完整快照。
- **success_criteria**：`git status` 可用；归档流程不变；CI/钩子可选。
