...
const Story = (() => {
  ...
  function _buildEntryCard(entry) {
    ...
    // 构建标题（支持双语显示）
    let titleHtml = '';
    if (entry.titleEN && entry.titleCN && entry.titleEN !== entry.titleCN) {
      // 有英文和中文，且不相同，显示双语
      titleHtml = `📖 <span class="story-title-en">${entry.titleEN}</span> <span class="story-title-cn">· ${entry.titleCN}</span>`;
    } else {
      // 只有中文或英文与中文相同，单语显示
    ...
    // 构建正文（支持双语显示）
    let textHtml = '';
    if (entry.bodyEN && entry.bodyCN && entry.bodyEN !== entry.bodyCN) {
      // 有英文和中文，且不相同，显示双语
      const bodyENHtml = entry.bodyEN
        .split('\n\n')
        .map(p => `<p class="story-text-en">${p.replace(/\n/g, '<br>')}</p>`)
        .join('');
      const bodyCNHtml = entry.bodyCN
        .split('\n\n')
        .map(p => `<p class="story-text-cn">${p.replace(/\n/g, '<br>')}</p>`)
        .join('');
      textHtml = bodyENHtml + bodyCNHtml;
    } else {
      // 只有中文或英文与中文相同，单语显示
      textHtml = entry.text
        .split('\n\n')
  ...
  function _buildLockedCard(entry) {
    ...
    const previewTitle = preview.hiddenTitle || entry.titleCN || entry.title || '未解锁故事';
  ...
  function checkUnlocks() {
    ...
    STORY_ENTRIES.forEach(entry => {
      ...
      if (orderOk && bondOk && statsOk && phaseOk && coinsOk) {
