...
const DEFAULT_STATE = {
  ...
  /* Giftable product sample inventory (keyed by product/recipe id) */
  inventory: {
    productSamples: {},
  },
  ...
  /* Narrative-only statistics; not shown in the base stats panel */
  storyStats: {
    lostOrdersByRole: {},
...
function addProductSamples(productId, amount = 1) {
  if (!productId || amount <= 0) return 0;
  _ensureInventory();
  State.inventory.productSamples[productId] = getProductSampleCount(productId) + amount;
  saveState();
  return State.inventory.productSamples[productId];
}

function useProductSample(productId, amount = 1) {
  if (!productId || amount <= 0) return false;
  _ensureInventory();
  if (getProductSampleCount(productId) < amount) return false;
  State.inventory.productSamples[productId] -= amount;
  saveState();
  return true;
}

function getProductSampleCount(productId) {
  if (!productId) return 0;
  _ensureInventory();
  return Number(State.inventory.productSamples[productId]) || 0;
}
...
/* ─── STORY STATS ─── */
function ensureStoryStats() {
  if (!State.storyStats || typeof State.storyStats !== 'object') {
    State.storyStats = JSON.parse(JSON.stringify(DEFAULT_STATE.storyStats));
  }
  if (!State.storyStats.lostOrdersByRole || typeof State.storyStats.lostOrdersByRole !== 'object') {
    State.storyStats.lostOrdersByRole = {};
  }
  if (!State.storyStats.lostOrdersByIdentity || typeof State.storyStats.lostOrdersByIdentity !== 'object') {
    State.storyStats.lostOrdersByIdentity = {};
  }
  return State.storyStats;
}

function recordLostOrderStoryStats(order) {
  if (!order || !order.customer) return null;
  const storyStats = ensureStoryStats();
  const customer = order.customer;
  const roleId = customer.bondRoleId || null;
  const identityId = getCustomerBaseIdentityId(customer);

  if (roleId) {
    storyStats.lostOrdersByRole[roleId] = (storyStats.lostOrdersByRole[roleId] || 0) + 1;
  }
  if (identityId) {
    storyStats.lostOrdersByIdentity[identityId] = (storyStats.lostOrdersByIdentity[identityId] || 0) + 1;
  }

  return { roleId, identityId };
}

function getStoryStatCount(bucket, key) {
  if (!bucket || !key) return 0;
  const storyStats = ensureStoryStats();
  return Number(storyStats[bucket] && storyStats[bucket][key]) || 0;
}

function getCustomerBaseIdentityId(customer) {
  if (!customer) return null;
  if (customer.identityId) return customer.identityId;
  if (customer._identityId) return customer._identityId;
  if (customer._baseCustomerId && CUSTOMER_IDENTITIES && CUSTOMER_IDENTITIES[customer._baseCustomerId]) {
    return customer._baseCustomerId;
  }
...
function getCustomerIdentityId(customer) {
  ...
  return getCustomerBaseIdentityId(customer);
}
...
function _ensureInventory() {
  if (!State.inventory || typeof State.inventory !== 'object') {
    State.inventory = JSON.parse(JSON.stringify(DEFAULT_STATE.inventory));
  }
  if (!State.inventory.productSamples || typeof State.inventory.productSamples !== 'object') {
    State.inventory.productSamples = {};
  }
