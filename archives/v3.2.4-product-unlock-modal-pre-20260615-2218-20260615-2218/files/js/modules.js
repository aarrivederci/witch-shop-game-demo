...
const Modules = (() => {
  ...
  function _tryUnlockRecipe(recipe, cost) {
    ...
    UI.showToast(_formatRecipeUnlockToast(recipe, sampleCount), 'success');
    render();
    UI.refreshHeader();
  }
  ...
  function _formatRecipeUnlockToast(recipe, sampleCount = 0) {
    const lines = [`✨ 解锁配方：${recipe.name}！`];
    if (recipe.intro) lines.push(recipe.intro);
    if (recipe.giftable) {
      lines.push(sampleCount > 0
        ? `获得商品样品 ×${sampleCount}，可免费赠送给熟客。`
        : '可作为商品样品赠送给熟客。');
    }
    return lines.join('\n');
  }
