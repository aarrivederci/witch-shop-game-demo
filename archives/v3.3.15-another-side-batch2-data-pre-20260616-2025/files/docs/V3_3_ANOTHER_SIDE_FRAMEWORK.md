...
第二批 `another side` 建议先作为规划稿沉淀，不在同一轮直接扩张运行时数据。目标是确认角色主题、条件边界与 QA 口径，再进入正式数据接入。

推荐拆分：

- 正式样例候选：`vampire_another_side_01`、`necromancer_another_side_01`、`ghost_another_side_01`。
- 后续候选：`elf_another_side_01`、`dragon_another_side_01`。
- 本批仍沿用 `private_workroom` + 角色羁绊 Lv.2 + 角色/职业分类记录的组合条件。
- 本批不新增角色 ID，不引入新统计 bucket，不改动当前运行时结构。
...
- 新增数据前先确认目标角色已存在于特殊 NPC / 羁绊体系中。
- `preview.visibleHints` 继续使用“隐秘工坊 / 更深信任 / 相关委托记录”这类玩家可读提示。
- 如果第二批正式接入运行时数据，必须通过 `node tools/check_another_side_stories.js` 与 `node tools/project_check.js`。
- 如果只做规划文档，本轮验收重点是边界清晰、候选 ID 可追踪、后续实施路径明确。