...
当前项目处于 **v3.0 大版本规划下的 v3.2 商品赠礼与特殊 NPC 关系循环运行时落地准备阶段**。

最新上下文：角色故事正文已完成“中文优先”运行时清理。`js/data.js` 的 `ROLE_STORY_LIBRARY` 已从旧四元组 `[titleEN, titleCN, bodyEN, bodyCN]` 转为中文二元组 `[title, body]`；`js/story.js` 只渲染 `title/text`；旧英文标题与正文已归档到 `docs/STORY_EN_ARCHIVE.md`。店铺投资故事已接入 `unlockShopTraits`，未解锁故事卡片会展示店铺气质条件；羁绊页“下个故事”提示也已支持订单、羁绊、阶段、金币、消费统计、店铺气质与送礼条件等完整条件摘要。礼物驱动故事条件已接入：`State.bonds[roleId]` 会记录总送礼数、指定礼物计数、最近礼物与重复赠礼衰减；`STORY_ENTRIES` 可使用 `unlockGifts` 要求“累计送礼”或“赠送指定角色标志礼物”；故事页与羁绊页均会展示这些条件。`tools/handoff_status.js` 已改为从 `AI_HANDOFF.md` / `CHANGELOG.md` 动态提取当前阶段、推荐下一步与 Unreleased 亮点，避免交接状态再次停留在旧版本。2026-06-15 已再次校准 v3.2：下一步不再优先把“经营消耗品 / 风险控制”作为唯一收尾，而是先把商品介绍、商品赠礼、专精品质、一般/特殊 NPC 分层、特殊 NPC 羁绊来访台词与羁绊回馈提示打成最小闭环。详细计划见 `docs/V3_2_PRODUCT_GIFT_SPECIAL_NPC_PLAN.md`。v3.2.1 文档与数据契约整理已完成：`docs/DATA_CONTRACTS.md` 已明确 `BOND_GIFTS` 兼容、商品赠礼字段、`State.bonds[roleId]` 扩展、一般/特殊 NPC 分层、特殊 NPC 订单与羁绊回馈契约；计划文档也已记录关键决策。`requirements` 统一、半隐藏预告、流失订单叙事入口、完整 IF 网络、特殊 NPC 专属订单链仍后置到 v3.3 或更后。如未来推进 v3.4 多语言词库包，可从英文归档恢复/拆分资源。
...
建议先按 `docs/V3_2_PRODUCT_GIFT_SPECIAL_NPC_PLAN.md` 推进 v3.2.2：商品数据补介绍与赠礼标签；兼容旧 `BOND_GIFTS`；先让已解锁商品能作为 `productGift` 赠送并接入偏好与专精系数。随后进入 v3.2.3：订单顾客区分一般 NPC 与特殊 NPC；特殊 NPC 使用已有 20 个具名角色并根据羁绊等级显示不同来访台词；同一时间最多 1 个特殊 NPC 订单，失败回退一般 NPC。羁绊阶段回馈第一版先做 `noteOnly` 记录和提示。暂不继续新增故事条件字段、暂不重构故事预告、暂不扩写流失订单/IF 故事网络、暂不做完整特殊委托链。
