...
const Story = (() => {
  ...
  function _getGiftUnlockHints(condition = {}, fallbackRoleId = null) {
    ...
    if (condition.count != null) {
      ...
      hints.push({ label: '赠送礼物', current, required, ok: current >= required });
    }
    ...
    if (condition.giftId) {
      ...
      hints.push({ label: gift ? `赠送${gift.name}` : '赠送指定礼物', current, required, ok: current >= required });
    }
    ...
    if (condition.productId) {
      ...
      hints.push({ label: product ? `赠送${product.name}样品` : '赠送指定商品样品', current, required, ok: current >= required });
    }
    ...
    if (condition.giftTag) {
      ...
      hints.push({ label: '赠送契合礼物', current, required, ok: current >= required });
    }
