...
建议先收尾 v3.2.2 剩余项：商品首次解锁介绍、必要的 `intro` 文案与（如要实现）`sampleOnUnlock` 样品库存；同时补一个最小手动 QA 说明。然后进入 v3.2.3：订单顾客区分一般 NPC 与特殊 NPC；特殊 NPC 使用已有 20 个具名角色并根据羁绊等级显示不同来访台词；同一时间最多 1 个特殊 NPC 订单，失败回退一般 NPC。羁绊阶段回馈第一版先做 `noteOnly` 记录和提示。暂不继续新增故事条件字段、暂不重构故事预告、暂不扩写流失订单/IF 故事网络、暂不做完整特殊委托链。
...
1. 商品已进入赠礼与角色关系循环，但商品首次解锁介绍与样品库存仍未完成。
