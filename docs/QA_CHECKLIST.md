...
- [ ] v3.3.20 another side UI QA：骑士、恶魔、花仙子、吸血鬼、死灵法师、幽灵角色页应显示 `Another Side` 专区与 `Another Side x/y` 状态，不被普通故事锁定顺序遮住。
- [ ] v3.3.20 another side 锁定/解锁 QA：未达成条件时显示半隐藏预告与玩家可读进度；执行对应 `WitchDebug.prepareAnotherSide(roleId)` 后，专用卡显示 `Another Side · 已显影` 并可阅读正文。
- [ ] v3.3.21 角色档案 QA：故事页不再显示“故事 / 记录”全局切换；单个角色下通过 `正篇 / 另一面` 模块查看该角色全部故事内容。
- [ ] v3.3.21 模块隔离 QA：`正篇` 只显示普通角色故事，`另一面` 只显示该角色的 `another_side` 锁定或已显影卡片，二者不混排。
- [ ] v3.3.21 小店成就 QA：统计页显示 `经营数据 / 小店成就` 二级切换；小店成就展示职业/角色阶段记录，且不直接暴露 `lostOrdersByRole`、`lostOrdersByIdentity` 等后台字段名。
- [ ] 职业/角色分类流失计数不出现在当前统计数据面板；后续应进入成就页 / 记录页并以阶段成就方式展示。
- [ ] 成就页 / 记录页展示职业或角色阶段记录时，不直接暴露 `lostOrdersByRole`、`lostOrdersByIdentity` 等后台字段名。
- [ ] 故事页不再承载记录视图；职业/角色阶段记录应从统计页“小店成就”进入。
