...
## 5.2 特殊委托静态样例契约（v3.3.18）

`SPECIAL_COMMISSIONS` 从 v3.3.18 起作为静态样例数据存在，用于承接高羁绊、店铺状态、前置故事与商品标签形成的主题委托链。当前版本不新增存档字段，不接入运行时任务系统，也不应把样例误展示为可正式接取的任务。
...
- `status: 'static_sample'` 表示该委托只用于数据契约与后续 UI/运行时设计参考。
- `requirements.bond.roleId` 应与 `roleId` 一致，且等级为正数。
- `requirements.shopInvestments`、`requirements.storyIds`、`objectives.identityId` 和 `objectives.tag` 必须引用现有数据。
- `objectives` 第一版只允许 `completeOrders` 与 `giftTag`，新增目标类型前必须同步检查脚本与 QA 清单。
- `rewards.noteOnly` 在静态样例期必须为 `true`；后续正式奖励接入前不要使用 `storyId` 直接解锁未实现故事。
- `preview` 必须是玩家可读线索，不应暴露内部字段名或精确后台 token。
