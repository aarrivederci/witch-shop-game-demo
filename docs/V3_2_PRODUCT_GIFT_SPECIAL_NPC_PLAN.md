...
- 每个特殊 NPC 的完整专属订单链。
- 每个商品对每个 NPC 的专属台词。
- 完整礼物库扩充：当前只接入首批商品样品赠礼最小闭环，未标记 `giftable` 的商品后续应结合 v3.3 故事、角色偏好和半隐藏线索分批纳入。
- 身份助力的复杂数值效果。
- 完整里世界 / IF 故事网络扩写。
- `requirements` 字段统一重构。
- 玩家自定义 NPC 或内容包导入。
...
礼物库边界说明（2026-06-15）：v3.2 不追求把所有商品都转为礼物，也不为每个商品补齐 NPC 专属台词。当前只验证“商品可赠礼、样品可消耗、羁绊可增长、旧礼物条件可兼容”的最小机制闭环。未进入礼物库的商品、角色专属偏好和礼物触发故事应作为 v3.3 内容网络的一部分，按模块与角色故事分批设计。

库存实施说明（2026-06-15 / v3.2.4）：`State.inventory.productSamples` 已接入最小闭环。手动解锁 `giftable` 商品时会按 `sampleOnUnlock` 发放样品，未显式配置时默认发放 1 份；羁绊页商品样品赠礼会优先消耗库存样品，无库存时才按商品赠礼成本扣金币。样品赠送仍记录 `productGifts / byProductId / byGiftTag` 与羁绊经验，但不计入金币消费统计或角色送礼花费。
...
当前状态：已完成最小闭环（商品赠礼运行时核心、商品首次解锁介绍与样品库存均已接入；模块专精系数后续可继续补强）。
...
- `js/state.js` 已新增 `inventory.productSamples` 与 `addProductSamples / useProductSample / getProductSampleCount`。
- `js/modules.js` 已在手动解锁可赠送商品时发放样品，未配置 `sampleOnUnlock` 时默认发放 1 份。
- `js/shop.js` 商品样品赠礼已优先消耗库存样品；无库存时才消耗金币。
...
- 手动解锁可赠送商品后能获得样品库存，首次赠送优先消耗样品且不扣金币。（待手动 QA；运行时代码已实现）
- 样品耗尽后再次赠送同一商品会回退为金币赠礼。（待手动 QA；运行时代码已实现）
...
当前状态：羁绊回馈提示与商品样品库存最小闭环已完成；QA 与路线文档已补充样品库存检查项。后续如果继续 v3.2.4 收尾，优先做手动 QA 与少量体验微调，不建议再扩张大系统。

交付物：

- 羁绊阶段回馈记录。
- 达到阶段时显示一次提示。
- 商品样品库存最小闭环：解锁发放、赠礼消耗、无库存金币回退。
...
- 至少 1 个可赠送商品解锁后发放样品；样品赠送不扣金币，样品耗尽后金币赠礼仍可用。
- 文档与检查脚本通过。
