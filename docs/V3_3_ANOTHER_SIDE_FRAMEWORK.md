...
## 10. v3.3.14 第二批角色规划 / v3.3.15 数据接入

第二批 `another side` 已按先规划、后接入的节奏推进：v3.3.14 先沉淀角色主题、条件边界与 QA 口径；v3.3.15 将正式样例候选写入运行时故事数据。

推荐拆分：

- 已接入正式样例：`vampire_another_side_01`、`necromancer_another_side_01`、`ghost_another_side_01`。
- 后续候选：`elf_another_side_01`、`dragon_another_side_01`。
- 本批仍沿用 `private_workroom` + 角色羁绊 Lv.2 + 角色/职业分类记录的组合条件。
- 本批不新增角色 ID，不引入新统计 bucket，只新增 `STORY_ENTRIES` 数据。

### 10.1 已接入正式样例

| 故事 ID | 角色 | 主题方向 | 推荐条件 | 正文边界 |
| --- | --- | --- | --- | --- |
| `vampire_another_side_01` | 吸血鬼 | 克制欲望与礼貌邀约 | `lostOrdersByRole.vampireMin: 1` | 写“想靠近但仍保持界限”，不写成因错过订单而来索取补偿。 |
| `necromancer_another_side_01` | 死灵法师 | 亡者与活人的边界 | `lostOrdersByRole.necromancerMin: 1` | 写“仍然回应的人”带来的动摇，不写成死亡惩罚或经营事故。 |
| `ghost_another_side_01` | 幽灵 | 被记住后的执念松动 | `lostOrdersByRole.ghostMin: 1` | 写名字、灯火与被听见，不写成店铺闹鬼事故。 |

### 10.2 后续候选

| 故事 ID | 角色 | 适合延后原因 | 后续接入方向 |
| --- | --- | --- | --- |
| `elf_another_side_01` | 精灵 | 更适合与自然、长寿、礼物标签联动后再写 | 可接自然类商品样品、长期羁绊或森林相关特殊委托。 |
| `dragon_another_side_01` | 幼龙 | 更适合与成长阶段、占有欲和特殊委托链联动 | 可接成长记录、宝物类商品或阶段委托。 |

### 10.3 第二批验收口径

- 目标角色已存在于特殊 NPC / 羁绊体系中，且不新增角色 ID。
- `preview.visibleHints` 继续使用“隐秘工坊 / 更深信任 / 相关委托记录”这类玩家可读提示。
- 第二批正式接入运行时数据后，必须通过 `node tools/check_another_side_stories.js` 与 `node tools/project_check.js`。
- 手动 QA 可分别执行 `WitchDebug.prepareAnotherSide('vampire')`、`WitchDebug.prepareAnotherSide('necromancer')`、`WitchDebug.prepareAnotherSide('ghost')`，确认故事页对应条目可解锁并阅读。