...
```bash
node tools/create_archive.js --name <version-name> --files <path1,path2,...>
```

或创建完整快照：

```bash
node tools/create_archive.js --name <version-name> --full
```

4. 确认本轮可能被修改的关键文件已复制到 `archives/<version>/files/`，或完整快照已创建。
5. 完成代码或文档修改。
6. 运行必要验证，例如：
...
- 用户只需要记住 `START_HERE.md`；新模型应由该入口继续阅读其他文档。
...
这些脚本默认从仓库根目录运行，并默认面向 `witch-shop-game/` 项目目录。