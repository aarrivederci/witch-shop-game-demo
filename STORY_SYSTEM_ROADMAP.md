...
里故事也不应只理解为“羁绊等级更高后的下一章”。v3.3.8 后需要特别区分：角色 `another side` 是乙女向角色侧面故事，店铺经营、流失订单或异常经营不能被直接写成它的剧情发生原因。经营与订单记录可以作为后台判定指标，正文应聚焦角色在关系中的另一面。
...
- 特殊配方；
- 角色关系事件；
- 角色/职业维度的叙事统计，例如某个职业相关订单的特殊记录。
- 隐秘工坊等 side story 总入口。
- 金币消费方向，例如长期购买某类主题礼物、投资神秘风格店铺、频繁使用失败保护。

### 2.2.1 Another Side / Side Story 特别规范

`another side` 推荐作为 `scope: 'character'`、`route: 'inner'` 的角色侧面故事处理。它可以表现角色的占有欲、保护欲、脆弱、执念、嫉妒、依赖、压抑疲惫或更亲密的关系边界，但不应被写成“店铺经营失败后角色来追责”的经营事故分支。

统一规则：

- `private_workroom` / 隐秘工坊是 `another side` 与 side story 的推荐总入口。
- 流失订单是后台判定指标，不是剧情因果。
- 角色 side story 应优先读取角色/职业分类统计，例如 `storyStats.lostOrdersByRole` 或 `storyStats.lostOrdersByIdentity`，不优先读取全局 `stats.customersLost`。
- 隐秘工坊开启后，购买前已经累计的分类记录也可以被追溯计入。
- 分类记录的玩家可见展示应放入成就页 / 记录页，而不是当前统计页。

错误示例：

```text
因为店铺流失了很多骑士订单，所以骑士来干预经营，并因此进入 another side。
```

推荐示例：

```text
玩家开启隐秘工坊，并与骑士建立足够信任；骑士相关委托中累积的“未抵达记录”达到阶段后，隐秘工坊映出他守则之外的保护欲与压抑疲惫。
```
...
店铺故事应承担“经营选择可见化”的功能。玩家把金币投向不同方向时，店铺不只是变强，也应变得更像某一种店：温柔可靠、神秘危险、奢华昂贵、偏向学术或偏向禁忌。后续可用店铺投资记录和经营统计形成店铺线故事条件。
...
requirements: {
  ...
  stats: {
    ...
    coinsSpentOnGiftsMin: 200,
    coinsSpentOnShopInvestmentsMin: 500,
    usedProtectionItemsMin: 2
  },
  shopTraits: [
    { trait: 'mystic', level: 2 }
  ],
  relationships: [
...
- 金币消费条件应优先表示玩家选择方向，例如礼物消费、店铺投资、风险控制，不建议只检查总金币数量。
- 店铺气质类条件应来自店铺投资或经营行为的累积结果，避免与单个升级按钮强绑定。

---

## 5. 流失订单、分类记录与成就页

流失订单不应只作为失败统计，也可以成为世界状态、店铺线、成就页记录和 side story 条件的一部分。但 v3.3.8 后必须区分两类用途：

```text
全局 customersLost：基础经营统计 / 店铺线 / 历史 IF 样例。
分类 lostOrdersByRole / lostOrdersByIdentity：角色或职业维度的成就记录与 another side 条件。
```

### 5.1 全局流失与店铺线

全局 `customersLost` 可以继续用于店铺故事、口碑、补救、经营 IF 或世界反馈。
...
route: 'if',
requirements: {
  stats: { customersLostMin: 20 }
}
```

用途：让经营失败不只是数值惩罚，也能产生店铺层面的叙事反馈。

### 5.2 分类流失与 another side

角色 side story 不建议直接读取全局 `customersLostMin`。更推荐新增叙事统计：
...
表现：这不是“店铺经营失败导致骑士剧情发生”，而是“隐秘工坊读取到骑士相关委托里积累的另一类记录”，从而允许玩家进入骑士守则之外的关系侧面。

### 5.3 成就页 / 记录页承接分类进度

职业或角色维度流失次数不进入当前统计页面。当前统计页保持基础经营概览：服务顾客、流失顾客、总按键、最高连击等。
...
这些阶段可以作为 side story 的半隐藏进度提示，但不应写成精确任务清单，例如“再失败 1 个骑士订单即可解锁”。
...
流失订单的设计定位不应固定为“玩家做错了所以扣口碑”。对店铺线来说，它可以是一种异常经营信号；对角色 another side 来说，它只是职业/角色维度的后台判定指标。

可选方向：

- 店铺线：可靠店铺、补救订单、传闻修复、口碑或经营分支。
- 世界线：异常顾客、隐藏委托、里世界传闻。
- 角色线：隐秘工坊开启后，用分类记录作为进入角色另一侧面的条件，不把订单流失写成正文因果。

因此，流失订单相关内容应避免只有“惩罚感”。即使它带来负面反馈，也应同时提供新的叙事信息、特殊顾客、成就阶段或角色侧面线索。
...
6. another side 不是店铺经营事故，隐秘工坊是入口，流失订单分类计数只是判定指标。
7. 流失顾客可以触发特殊故事，但不应只作为惩罚，应当提供另一种叙事反馈。
8. 中文正文优先，英文只作为必要别名或专有名词保留。
9. 修改打字系统时必须保持内置默认词库可用，导入词库只能作为增强能力，不能成为网页发布的前置条件。
