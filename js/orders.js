...
const Orders = (() => {
  ...
  function _completeOrder(order) {
    ...
    _showOrderBondStageRewards(order);
  ...
  function _showOrderBondStageRewards(order) {
    if (!order || !order.customer || typeof getCustomerIdentityId !== 'function') return;
    const roleId = getCustomerIdentityId(order.customer);
    if (!roleId || typeof getBondStageRewardHistory !== 'function') return;
    ...
    const roleName = (role && (role.charName || role.name)) || order.customer.charName || order.customer.name || '熟客';
  ...
  function _failOrder(order) {
    ...
    if (typeof recordLostOrderStoryStats === 'function') recordLostOrderStoryStats(order);
    saveState();
