...
const UI = (() => {
  ...
  function _renderStats() {
    ...
    container.innerHTML = `
      ...
      <div class="stats-view-tabs">
        <button class="stats-view-tab ${_activeStatsView === 'data' ? 'active' : ''}" data-stats-view="data">经营数据</button>
        <button class="stats-view-tab ${_activeStatsView === 'achievements' ? 'active' : ''}" data-stats-view="achievements">小店成就</button>
      </div>
      <div id="stats-view-content"></div>
    `;

    container.querySelectorAll('[data-stats-view]').forEach(btn => {
      btn.addEventListener('click', () => {
        _activeStatsView = btn.dataset.statsView;
        _renderStats();
      });
    });
    ...
    if (_activeStatsView === 'achievements') {
      if (typeof Story !== 'undefined' && Story.renderRecordsInto) {
        Story.renderRecordsInto(content);
      }
      return;
    }
  ...
  /* ─── PRODUCT UNLOCK CARD ─── */
  function showProductUnlockCard(recipe, sampleCount = 0) {
    const modal = document.getElementById('product-unlock-modal');
    ...
    const giftableHtml = recipe.giftable
      ? `
        <div class="product-unlock-samples">
          <span>📦 获得样品 <strong>×${sampleCount}</strong></span>
          <span>💝 可赠送给熟客</span>
          <span>✨ 羁绊基础 +${recipe.giftBaseExp || 1}</span>
          <span>🪙 无样品时回退 ${fallbackCost}</span>
        </div>
      `
      : '<div class="product-unlock-note">已加入可制作配方。</div>';

    content.innerHTML = `
      <div class="product-unlock-kicker">新商品解锁</div>
      <div class="product-unlock-icon">${recipe.icon || '✨'}</div>
      <h2 id="product-unlock-title">${recipe.name}</h2>
      <div class="product-unlock-module">${moduleLabel}</div>
      <p class="product-unlock-intro">${recipe.intro || recipe.desc || '新的魔法商品已经加入配方列表。'}</p>
      ${giftableHtml}
      <button type="button" class="pixel-btn product-unlock-confirm">知道了</button>
    `;

    _bindProductUnlockModal(modal);
    modal.classList.remove('hidden');
    modal.setAttribute('aria-hidden', 'false');
    requestAnimationFrame(() => modal.classList.add('show'));
  }

  function _bindProductUnlockModal(modal) {
    const close = () => _hideProductUnlockModal(modal);
    const closeBtn = modal.querySelector('.product-unlock-close');
    const confirmBtn = modal.querySelector('.product-unlock-confirm');
  ...
  function _hideProductUnlockModal(modal) {
