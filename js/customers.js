...
const Customers = (() => {
  ...
  const SPECIAL_NPC_SPAWN_CHANCE = 0.2;
  ...
  function _pickCustomer() {
    ...
    const specialCustomer = _tryPickSpecialCustomer(eligible);
    if (specialCustomer) return specialCustomer;
  ...
  function _tryPickSpecialCustomer(eligible) {
    if (_hasPendingSpecialNpcOrder()) return null;
    if (Math.random() >= SPECIAL_NPC_SPAWN_CHANCE) return null;
  ...
  function _hasPendingSpecialNpcOrder() {
    if (!Orders || typeof Orders.getOrders !== 'function') return false;
    return Orders.getOrders().some(order => (
      order && order.status === 'pending' && order.customer && order.customer.npcType === 'special'
    ));
  }

  function _buildSpecialCustomerVisit(baseCustomer) {
    const bondLevel = getBondLevel(baseCustomer.id);
    const bondLabel = _getBondLevelLabel(bondLevel);
    return {
      ...baseCustomer,
      id: `special_${baseCustomer.id}`,
      identityId: baseCustomer.id,
      name: baseCustomer.charName
        ? `${baseCustomer.charName} · ${baseCustomer.name}`
        : baseCustomer.name,
      npcType: 'special',
      special: true,
      bondRoleId: baseCustomer.id,
      bondLevel,
      bondLabel,
      dialogue: _getSpecialVisitLines(baseCustomer, bondLevel),
      _baseCustomerId: baseCustomer.id,
    };
  }

  function _asGeneralCustomer(baseCustomer) {
    return {
      ...baseCustomer,
      id: `general_${baseCustomer.id}_${Math.random().toString(36).slice(2, 5)}`,
      identityId: baseCustomer.id,
      npcType: 'general',
      special: false,
      bondRoleId: null,
  ...
  function _getSpecialVisitLines(customer, bondLevel) {
    const lines = customer.visitLines;
    if (!lines) return customer.dialogue;
    const key = `bond${Math.min(3, Math.max(0, bondLevel))}`;
    return lines[key] || lines.bond0 || customer.dialogue;
  }

  function _getBondLevelLabel(level) {
    const found = (BOND_LEVELS || []).find(item => item.level === level);
    return found ? found.label : '熟客';
  }
  ...
  function _buildOrder(customer, recipe) {
    ...
    const dialogue = _pickDialogue(customer);
    ...
    return {
      ...
      status:      'pending',
      dialogue,
      npcType:     customer.npcType || 'general',
      isSpecialNpc: customer.npcType === 'special',
      bondRoleId:  customer.bondRoleId || null,
      bondLevel:   customer.bondLevel || 0,
      bondLabel:   customer.bondLabel || '',
    };
  }
