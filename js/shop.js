...
const Shop = (() => {
  ...
  function _buildBondDetailPanel(role) {
    ...
    const commissionLead = _getRoleCommissionLead(role.id);
    ...
    panel.innerHTML = `
      ...
      ${_formatCommissionLead(commissionLead)}
  ...
  function _getRoleCommissionLead(roleId) {
    const list = (typeof SPECIAL_COMMISSIONS !== 'undefined' && SPECIAL_COMMISSIONS) || [];
    const commission = list.find(item => item && item.roleId === roleId);
    if (!commission) return null;
    const requirements = _getCommissionRequirementConditions(commission);
    const objectives = _getCommissionObjectiveConditions(commission);
    const allConditions = requirements.concat(objectives);
    return {
      commission,
      requirements,
      objectives,
      ready: requirements.length ? requirements.every(item => item.ok) : true,
      complete: allConditions.length ? allConditions.every(item => item.ok) : false,
    };
  }

  function _formatCommissionLead(lead) {
    if (!lead || !lead.commission) return '';
    const { commission, requirements, objectives, ready, complete } = lead;
    const preview = commission.preview || {};
    const hint = preview.hint || '柜台边似乎出现了一条尚未整理的特殊委托线索。';
    const visibleHints = (preview.visibleHints || [])
      .map(text => `<li>${text}</li>`)
      .join('');
    const requirementItems = requirements.length
      ? requirements.map(item => `<li class="${item.ok ? 'done' : ''}">${item.ok ? '✅' : '✦'} ${item.text}</li>`).join('')
      : '<li class="done">✅ 暂无额外前置</li>';
    const objectiveItems = objectives.length
      ? objectives.map(item => `<li class="${item.ok ? 'done' : ''}">${item.ok ? '✅' : '✦'} ${item.text}</li>`).join('')
      : '<li>后续版本会补充正式委托目标。</li>';

    return `
      <div class="bond-commission-lead ${ready ? 'ready' : ''} ${complete ? 'complete' : ''}">
        <div class="bond-commission-title">
          <span>🕯️ 特殊委托线索</span>
          <em>${complete ? '线索已备齐' : ready ? '前置已满足' : '调查中'}</em>
        </div>
        <div class="bond-commission-name">${commission.title || '未命名委托'}</div>
        <div class="bond-commission-hint">${hint}</div>
        ${visibleHints ? `<ul class="bond-commission-hints">${visibleHints}</ul>` : ''}
        <div class="bond-commission-progress">
          <div><strong>出现条件</strong><ul>${requirementItems}</ul></div>
          <div><strong>主题目标</strong><ul>${objectiveItems}</ul></div>
        </div>
        <div class="bond-commission-note">当前为静态样例：暂不接取、不发放奖励，只用于提前提示后续委托链方向。</div>
      </div>
    `;
  }

  function _getCommissionRequirementConditions(commission) {
    const requirements = commission.requirements || {};
    const roleId = commission.roleId;
    const conditions = [];

    if (requirements.bond) {
      conditions.push(_getStoryBondCondition(requirements.bond, roleId));
    }
    (Array.isArray(requirements.shopInvestments) ? requirements.shopInvestments : [requirements.shopInvestments])
      .filter(Boolean)
      .forEach(id => {
        const investment = (SHOP_INVESTMENTS || []).find(item => item.id === id);
        const ok = isShopInvestmentPurchased(id);
        conditions.push({
          text: ok ? `已准备${investment ? investment.name : '关键店铺投资'}` : `准备${investment ? investment.name : '关键店铺投资'}`,
          ok,
        });
      });
    (Array.isArray(requirements.storyIds) ? requirements.storyIds : [requirements.storyIds])
      .filter(Boolean)
      .forEach(id => {
        const story = (STORY_ENTRIES || []).find(item => item.id === id);
        const ok = State.storyUnlocked.includes(id);
        conditions.push({
          text: ok ? `已读过${story ? story.title : '相关侧面故事'}` : `留意${story ? story.title : '相关侧面故事'}的线索`,
          ok,
        });
      });
    return conditions;
  ...
  function _getCommissionObjectiveConditions(commission) {
    return (commission.objectives || []).map(objective => {
      if (objective.type === 'completeOrders') {
        const role = CUSTOMER_TYPES.find(item => item.id === objective.identityId);
        const current = getOrderCompletionCount({ identity: objective.identityId, personality: role && role.personality });
        const required = Math.max(1, Number(objective.count) || 1);
        return {
          text: `完成相关熟客订单 ${Math.min(current, required)} / ${required}`,
          ok: current >= required,
        };
      }
      if (objective.type === 'giftTag') {
        const current = getBondGiftTagCount(objective.roleId || commission.roleId, objective.tag);
        const required = Math.max(1, Number(objective.count) || 1);
        return {
          text: `送出契合“${_getGiftTagLabel(objective.tag)}”的商品样品 ${Math.min(current, required)} / ${required}`,
          ok: current >= required,
        };
      }
      return {
